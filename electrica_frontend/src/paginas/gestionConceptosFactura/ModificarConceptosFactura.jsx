...
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
// - Componente para modificar los parámetros aplicables a las facturas -
// ----------------------------------------------------------------------
// ----------------------------------------------------------------------
const ModificarConceptosFactura = () => {
  ...
  const [conceptosFacturaId, setConceptosFacturaId] = useState(null);
  ...
  // -------------------------------------------------------------------------------
  // - UseEffect para conseguir los valores de los conceptos de factura en la BBDD -
  // -------------------------------------------------------------------------------
  useEffect(() => {
    const conseguirConceptosFactura = async () => {
      const conceptosFactura = await axiosPrivado.get("/api/conceptosFactura");
      setConceptosFacturaId(conceptosFactura.data[0].id);
      setPrecioPotencia(conceptosFactura.data[0].precioPotencia);
      setImpuestoElectricidad(conceptosFactura.data[0].impuestoElectricidad);
      setAlquilerEquipos(conceptosFactura.data[0].alquilerEquipos);
      setIva(conceptosFactura.data[0].iva);
    };
  ...
  // -------------------------------------------------------------
  // - Método para modificar los conceptos de factura en la BBDD -
  // -------------------------------------------------------------
  const handleAceptar = async (e) => {
    ...
    try {
      ...
      await axiosPrivado.patch(
        `/api/conceptosFactura/${conceptosFacturaId}`,
        nuevoRegistro
      );
      dispatch(setMensajeAMostrar("Registro modificado correctamente"));
  ...
  return (
    <Contenedor>
      ...
      <Form onSubmit={handleAceptar}>
        <Titulo>Modificar Parámetros</Titulo>
        <Linea>
...
export default ModificarConceptosFactura;
