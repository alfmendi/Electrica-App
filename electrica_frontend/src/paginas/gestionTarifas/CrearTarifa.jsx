...
import { crearTarifa } from "../../redux/tarifasSlice";
...
// -----------------------------------------------------
// -----------------------------------------------------
// - Componente para añadir una nueva tarifa a la BBDD -
// -----------------------------------------------------
// -----------------------------------------------------
const CrearTarifa = () => {
  ...
  // ---------------------------------------------------------
  // - UseEffect para comprobar si ya existe una tarifa base -
  // ---------------------------------------------------------
  useEffect(() => {
    const buscarTarifaBase = tarifas.find(
      (tarifa) => tarifa.tarifaBase === true
    );
    setExisteTarifaBase(buscarTarifaBase ? true : false);
  }, []);
  ...
  // ----------------------------------------------
  // - Método para almacenar la tarifa en la BBDD -
  // ----------------------------------------------
  const handleAceptar = async (e) => {
    ...
    try {
      ...
      const respuesta = await axiosPrivado.post("/api/tarifas", nuevaTarifa);
      dispatch(crearTarifa(respuesta.data));
      dispatch(setMensajeAMostrar("Tarifa creada correctamente"));
      navigate("/tarifas");
    } catch (error) {
          ...
          <Linea style={{ marginBottom: "1rem" }}>
            ...
            >
              <Switch
                value={tarifaBase}
                checked={tarifaBase}
                onChange={() => setTarifaBase(!tarifaBase)}
              />
            </div>
          </Linea>
        )}
