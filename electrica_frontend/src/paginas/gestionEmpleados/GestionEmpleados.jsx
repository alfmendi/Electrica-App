...
import { conseguirEmpleados } from "../../redux/empleadosSlice";
...
const GestionEmpleados = () => {
  ...
  // ---------------------------------------------------------------------
  // - UseEffect para conseguir todos los empleados presentes en la BBDD -
  // ---------------------------------------------------------------------
  useEffect(() => {
    if (!opcionActiva) {
    ...
    const obtenerEmpleados = async () => {
      try {
        ...
        const respuesta = await axiosPrivado.get("/api/empleados");
        isMounted && dispatch(conseguirEmpleados(respuesta.data));
      } catch (error) {
        generarMensajeError(error);
      }
    };
    // Solo ejecuto este método cuando empleados está vacio.
    // Esto evita tener que estar permanentemente leyendo los empleados
    // de la BBDD cada vez que entro en esta página
    if (empleados.length === 0) obtenerEmpleados();
    return () => {
      isMounted = false;
    };
  }, []);
  ...
  // ------------------------------------------------------------------------------------
  // - UseEffect para conseguir todos los empleados que cumplen la condición del filtro -
  // ------------------------------------------------------------------------------------
  useEffect(() => {
    if (nombreEmpleado !== "") {
      setEmpleadosFiltro(
        empleados.filter((elemento) =>
          elemento.nombre.toLowerCase().includes(nombreEmpleado.toLowerCase())
        )
      );
      setBuscar(true);
    } else {
  ...
  return (
    <Contenedor>
      ...
      <PanelBotones>
        ...
        <Boton>
          <StyledLink to={"/empleados/crear"}>Crear Empleado</StyledLink>
        </Boton>
      </PanelBotones>
      <PanelBuscar>
